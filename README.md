...
### If will run this project in a CI pay attention to the command below:
 > Note: Be careful with this command, it will apply changes to your cluster.

`make ci`

This command, will install kubectl, configure kube config file and run `./scripts/deployer.sh`, this script will perform a blue green deployment.

 > Note: You must have an existent deployment.
...
escrever como rodar um usuário novo
escrever como setei o letsencrypt
kops
colocar links que utilizei 
explicar as roles tudo bonitnho
Por que do Makefile
Melhorar o deployer.sh para limpar deployments "quebrados" em produção
explicar o motivo que dessiti do jenkins, mas a rola tá lá - done


Explicar por que o deploy e atualização do prometheus pelo ansible // Vault e que eu tenho um repo que faz deploy automaticamente do prometheus


explicar o motivo de usar as configs da maquina diretasso no ansible - não encontrei melhor forma ainda de fazer esse "setup" provavelmente é melhor fazer com o terraform - todo




// Fazer feature toggle de count de documentos para que minha API não seja devastada e meu storage também. - todo

// Deixar claro o motivo de não rodar um mongodb no kubernetes. done

// explicar o motivo desta arquitetura. done

// falar sobre backups - todo

//melhoria do uso de environments e secrets - todo vault

// gostaria de ter bots no slack - done

// alertas no slack e pushover - todo

// graylogs - done

// lint no CI e lint nos PR - done
